...
function ShopInfoDialog({
    ...
    const Row = ({ label, value }: { label: string; value: React.ReactNode }) => (
        <Box sx={{ display: 'flex', justifyContent: 'space-between', py: 1, borderBottom: `1px solid ${theme.palette.divider}` }}>
            <Typography sx={{ fontSize: 13, color: 'text.secondary', minWidth: 140 }}>{label}</Typography>
    ...
    return (
        <Dialog open={open} onClose={onClose} maxWidth="sm" fullWidth PaperProps={{ sx: { borderRadius: 3 } }}>
            <DialogTitle sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', pb: 1, borderBottom: `1px solid ${theme.palette.divider}` }}>
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
                    <Store sx={{ color: theme.palette.primary.main }} />
                    <Typography fontWeight={700} fontSize={16}>Shop Information</Typography>
                </Box>
            ...
            <DialogContent sx={{ pt: 2.5 }}>
                ...
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 2, mb: 3, p: 2, bgcolor: '#F8FAFC', borderRadius: 2 }}>
                    <Avatar src={shop.logoUrl} sx={{ width: 56, height: 56, borderRadius: 2, fontSize: 22, bgcolor: '#E2E8F0' }}>
                        {shop.shopName[0]}
                    </Avatar>
                    <Box sx={{ flex: 1 }}>
                        <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.75 }}>
                            <Typography fontWeight={700} fontSize={15}>{shop.shopName}</Typography>
                            {shop.isVerified && <VerifiedUser sx={{ fontSize: 16, color: '#3B82F6' }} />}
                        </Box>
                        <Typography fontSize={12} color="text.secondary">{shop.shopCode}</Typography>
                        <Chip label={shop.tier} size="small" sx={{ mt: 0.5, height: 18, fontSize: 10, fontWeight: 600, bgcolor: '#FEF3C7', color: '#92400E' }} />
                    </Box>
                    {shop.avgRating != null && (
                        <Box sx={{ textAlign: 'center' }}>
                            <Box sx={{ display: 'flex', alignItems: 'center', gap: 0.5 }}>
                                <Star sx={{ fontSize: 16, color: '#F59E0B' }} />
                                <Typography fontWeight={700} fontSize={15}>{shop.avgRating.toFixed(1)}</Typography>
                            </Box>
                ...
                <Grid container spacing={2}>
                    <Grid size={{ xs: 12, md: 6 }}>
                        <Typography fontSize={11} fontWeight={700} color="text.secondary" sx={{ mb: 1, textTransform: 'uppercase', letterSpacing: 0.5 }}>Contact</Typography>
                    ...
                    <Grid size={{ xs: 12, md: 6 }}>
                        <Typography fontSize={11} fontWeight={700} color="text.secondary" sx={{ mb: 1, textTransform: 'uppercase', letterSpacing: 0.5 }}>Stats</Typography>
                        <Row label="Total Products" value={String(shop.totalProducts ?? '—')} />
                        <Row label="Total Orders" value={String(shop.totalOrders ?? '—')} />
                        <Row label="Commission" value={`${((shop.commissionRate ?? 0) * 100).toFixed(1)}%`} />
                        <Row label="Joined" value={formatDate(shop.joinedAt)} />
                    </Grid>
                    <Grid size={{ xs: 12 }}>
                        <Typography fontSize={11} fontWeight={700} color="text.secondary" sx={{ mb: 1, textTransform: 'uppercase', letterSpacing: 0.5 }}>Location</Typography>
                        <Row label="Address" value={shop.address} />
                        <Row label="Ward" value={shop.wardName} />
                        <Row label="District" value={shop.districtName} />
                        <Row label="Province" value={shop.provinceName} />
                    </Grid>
                </Grid>
            </DialogContent>
        </Dialog>
...
function ProductDetailDialog({
    ...
    const Row = ({ label, value }: { label: string; value: React.ReactNode }) => (
        <Box sx={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', py: 0.75 }}>
            <Typography sx={{ fontSize: 13, color: 'text.secondary', minWidth: 160 }}>{label}</Typography>
            {typeof value === 'string'
    ...
    const SectionLabel = ({ title }: { title: string }) => (
        <Typography sx={{ fontSize: 11, fontWeight: 700, color: 'text.secondary', textTransform: 'uppercase', letterSpacing: 0.8, mb: 1.5, mt: 2 }}>
            {title}
        </Typography>
    );
    ...
    const gi = item.groupInfo;

    // Reset img index when dialog opens for a new item
            ...
            <DialogTitle sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', borderBottom: `1px solid ${theme.palette.divider}`, py: 2 }}>
                <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
                    ...
                    {item.verificationType === 'UPDATE' && (
                        <Chip label="Update Request" size="small" sx={{ height: 20, fontSize: 10, fontWeight: 600, bgcolor: '#FEF3C7', color: '#92400E' }} />
                    )}
            ...
            <DialogContent sx={{ px: 3, py: 2.5 }}>
                <Grid container spacing={3}>
                    ...
                    <Grid size={{ xs: 12, md: 5 }}>
                        <Box sx={{ borderRadius: 2, overflow: 'hidden', height: 220, position: 'relative', bgcolor: '#F1F5F9', border: `1px solid ${theme.palette.divider}` }}>
                            {item.productImages.length > 0 ? (
                                <Box component="img" src={item.productImages[imgIdx]} sx={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                            ) : (
                            ...
                            {item.productImages.length > 1 && (
                                <>
                                    <Box onClick={() => setImgIdx(p => (p - 1 + item.productImages.length) % item.productImages.length)}
                                        sx={{ position: 'absolute', left: 8, top: '50%', transform: 'translateY(-50%)', width: 28, height: 28, borderRadius: '50%', bgcolor: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', color: '#fff', fontSize: 18 }}>
                                        ‹
                                    </Box>
                                    <Box onClick={() => setImgIdx(p => (p + 1) % item.productImages.length)}
                                        sx={{ position: 'absolute', right: 8, top: '50%', transform: 'translateY(-50%)', width: 28, height: 28, borderRadius: '50%', bgcolor: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', color: '#fff', fontSize: 18 }}>
                                        ›
                                    </Box>
                                </>
                            )}
                        ...
                        {item.productImages.length > 1 && (
                            <Box sx={{ display: 'flex', gap: 0.75, mt: 1 }}>
                                {item.productImages.slice(0, 4).map((img, i) => (
                                    <Box key={i} component="img" src={img} onClick={() => setImgIdx(i)}
                                        sx={{ width: 56, height: 44, objectFit: 'cover', borderRadius: 1.5, cursor: 'pointer', border: i === imgIdx ? `2px solid ${theme.palette.primary.main}` : `1px solid ${theme.palette.divider}` }} />
                                ))}
                            </Box>
                        )}

                        {/* Pricing */}
                        <Paper elevation={0} sx={{ mt: 2, p: 2, borderRadius: 2, bgcolor: '#F8FAFC', border: `1px solid ${theme.palette.divider}` }}>
                            <SectionLabel title="Pricing & Stock" />
                            <Row label="Base Price" value={<Typography sx={{ fontSize: 15, fontWeight: 700, color: theme.palette.primary.main }}>{formatPrice(item.basePrice)}</Typography>} />
                        ...
                        {/* For UPDATE type: show what's changing */}
                        {item.verificationType === 'UPDATE' && item.proposedChangesJson && (
                            <Paper elevation={0} sx={{ mt: 2, p: 2, borderRadius: 2, bgcolor: '#FFFBEB', border: `1px solid #FDE68A` }}>
                                <SectionLabel title="⚠ Proposed Changes" />
                                <Typography fontSize={12} color="#92400E" sx={{ mb: 1 }}>
                                    This is an update request. The live product is unchanged until approved.
                                </Typography>
                                <Box component="pre" sx={{ fontSize: 11, fontFamily: 'monospace', bgcolor: '#fff', p: 1.5, borderRadius: 1, border: '1px solid #FDE68A', overflowX: 'auto', maxHeight: 200, whiteSpace: 'pre-wrap', wordBreak: 'break-all' }}>
                                    {JSON.stringify(JSON.parse(item.proposedChangesJson), null, 2)}
                                </Box>
                            </Paper>
                        )}
                    </Grid>

                    {/* Right: group + variant info */}
                    <Grid size={{ xs: 12, md: 7 }}>
                        <Paper elevation={0} sx={{ p: 2, borderRadius: 2, bgcolor: '#F8FAFC', border: `1px solid ${theme.palette.divider}`, mb: 2 }}>
                            <SectionLabel title={frame ? 'Frame Group' : 'Accessory Group'} />
                            {frame ? (
                                <>
                                    <Row label="Frame Name" value={(gi as FrameGroupInfo).frameName} />
                                    <Row label="Shape · Structure" value={`${(gi as FrameGroupInfo).frameShape} · ${(gi as FrameGroupInfo).frameStructure}`} />
                                    <Row label="Material" value={(gi as FrameGroupInfo).frameMaterial} />
                                    <Row label="Gender · Age" value={`${(gi as FrameGroupInfo).genderTarget} · ${(gi as FrameGroupInfo).ageGroup}`} />
                                    <Box sx={{ display: 'flex', gap: 1, mt: 1, flexWrap: 'wrap' }}>
                                        {(gi as FrameGroupInfo).hasNosePads && <Chip label="Nose Pads" size="small" sx={{ fontSize: 10, height: 18, bgcolor: '#DCFCE7', color: '#166534' }} />}
                                        {(gi as FrameGroupInfo).hasSpringHinge && <Chip label="Spring Hinge" size="small" sx={{ fontSize: 10, height: 18, bgcolor: '#DCFCE7', color: '#166534' }} />}
                                        {(gi as FrameGroupInfo).vrEnabled && <Chip label="VR Enabled" size="small" sx={{ fontSize: 10, height: 18, bgcolor: '#EFF6FF', color: '#1D4ED8' }} icon={<ViewInAr sx={{ fontSize: 12 }} />} />}
                                    </Box>
                                    {(gi as FrameGroupInfo).vrEnabled && (gi as FrameGroupInfo).model3dUrl && (
                                        <Button size="small" variant="outlined" startIcon={<ViewInAr />}
                                            href={(gi as FrameGroupInfo).model3dUrl!} target="_blank"
                                            sx={{ mt: 1.5, fontSize: 12, textTransform: 'none', borderRadius: 1.5 }}>
                                            View 3D Model
                                        </Button>
                                    )}
                                    {(gi as FrameGroupInfo).description && (
                                        <Box sx={{ mt: 1.5 }}>
                                            <Typography fontSize={11} color="text.secondary" sx={{ mb: 0.25 }}>Description</Typography>
                                            <Typography fontSize={13}>{(gi as FrameGroupInfo).description}</Typography>
                                        </Box>
                                    )}
                                </>
                            ) : (
                                <>
                                    <Row label="Name" value={(gi as AccessoryGroupInfo).name} />
                                    <Row label="Type" value={(gi as AccessoryGroupInfo).type} />
                                    {(gi as AccessoryGroupInfo).description && (
                                        <Box sx={{ mt: 1 }}>
                                            <Typography fontSize={11} color="text.secondary">Description</Typography>
                                            <Typography fontSize={13} sx={{ mt: 0.25 }}>{(gi as AccessoryGroupInfo).description}</Typography>
                                        </Box>
                                    )}
                                </>
                            )}
                        ...
                        <Paper elevation={0} sx={{ p: 2, borderRadius: 2, bgcolor: '#F8FAFC', border: `1px solid ${theme.palette.divider}` }}>
                            <SectionLabel title="Variant Details" />
                            {frame ? (
                                <>
                                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 1.5 }}>
                                        <Box sx={{ width: 18, height: 18, borderRadius: '50%', bgcolor: (vi as FrameVariantInfo).colorHex, border: '2px solid rgba(0,0,0,0.1)', flexShrink: 0 }} />
                                        <Typography fontSize={13} fontWeight={600}>{(vi as FrameVariantInfo).colorName}</Typography>
                                        <Chip label={(vi as FrameVariantInfo).size} size="small" sx={{ height: 18, fontSize: 10, ml: 'auto' }} />
                                    </Box>
                                    <Box sx={{ fontFamily: 'monospace', fontSize: 11, color: 'text.secondary', bgcolor: '#F1F5F9', px: 1.5, py: 0.75, borderRadius: 1, mb: 1.5 }}>
                                        Frame {(vi as FrameVariantInfo).frameWidthMm}mm · Lens {(vi as FrameVariantInfo).lensWidthMm}×{(vi as FrameVariantInfo).lensHeightMm}mm · Bridge {(vi as FrameVariantInfo).bridgeWidthMm}mm · Temple {(vi as FrameVariantInfo).templeLengthMm}mm
                                    </Box>
                                    <Row label="Warranty" value={`${(vi as FrameVariantInfo).warrantyMonths} months`} />
                                    <Row label="Returnable" value={(vi as FrameVariantInfo).isReturnable ? '✅ Yes' : '❌ No'} />
                                    <Row label="Featured" value={(vi as FrameVariantInfo).isFeatured ? '⭐ Yes' : 'No'} />
                                </>
                            ) : (
                                <>
                                    {(vi as AccessoryVariantInfo).name && <Row label="Variant Name" value={(vi as AccessoryVariantInfo).name!} />}
                                    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, py: 0.75 }}>
                                        <Typography fontSize={13} color="text.secondary" sx={{ minWidth: 160 }}>Color</Typography>
                                        {(vi as AccessoryVariantInfo).colorHex && (
                                            <Box sx={{ width: 14, height: 14, borderRadius: '50%', bgcolor: (vi as AccessoryVariantInfo).colorHex!, border: '1px solid rgba(0,0,0,0.12)' }} />
                                        )}
                                        <Typography fontSize={13} fontWeight={600}>{(vi as AccessoryVariantInfo).color ?? '—'}</Typography>
                                    </Box>
                                    <Row label="Size" value={(vi as AccessoryVariantInfo).size ?? '—'} />
                                    <Row label="Warranty" value={`${(vi as AccessoryVariantInfo).warrantyMonths} months`} />
                                    <Row label="Returnable" value={(vi as AccessoryVariantInfo).isReturnable ? '✅ Yes' : '❌ No'} />
                                </>
                            )}
                        </Paper>

                        {/* Audit trail for reviewed items */}
                        {item.status !== 'PENDING' && (
                            <Paper elevation={0} sx={{ mt: 2, p: 2, borderRadius: 2, bgcolor: item.status === 'APPROVED' ? '#F0FDF4' : '#FEF2F2', border: `1px solid ${item.status === 'APPROVED' ? '#BBF7D0' : '#FECACA'}` }}>
                                <SectionLabel title="Review Decision" />
                                <Row label="Decision" value={<StatusChip label={item.status} vstatus={item.status} />} />
                                <Row label="Reviewed By" value={item.reviewedByName ?? '—'} />
                                <Row label="Reviewed At" value={formatDate(item.reviewedAt)} />
                                {item.status === 'REJECTED' && (
                                    <>
                                        <Row label="Reason" value={REJECT_REASONS.find(r => r.value === item.rejectionReason)?.label ?? item.rejectionReason ?? '—'} />
                                        {item.rejectionNote && (
                                            <Box sx={{ mt: 1 }}>
                                                <Typography fontSize={11} color="text.secondary">Notes</Typography>
                                                <Typography fontSize={13} sx={{ mt: 0.25, fontStyle: 'italic' }}>{item.rejectionNote}</Typography>
                                            </Box>
                                        )}
                                    </>
                                )}
